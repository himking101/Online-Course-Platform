import { AlertType } from '../../../../core/services/alert/alert.dto';
import { Component, EventEmitter, Input, Output } from '@angular/core';
import { linkify } from '../../../../core/services/utils/linkify';
...
export class Alert {
  @Input({ required: true }) alertType: AlertType = 'error';
  @Input({ required: true }) message: string = '';
  @Input({ required: true }) showAlert: boolean = false;
  @Output() showAlertChange = new EventEmitter<boolean>();

  get alertMessage(): string {
    return linkify(this.message, this.linkClassName);
  }

  get linkClassName(): string {
    switch (this.alertType) {
      case 'info':
        return 'text-blue-800 hover:text-blue-500';
      case 'success':
        return 'text-green-800 hover:text-green-500';
      case 'warning':
        return 'text-yellow-800 hover:text-yellow-500';
      case 'error':
